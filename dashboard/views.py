...
class OrganizationDashboardView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        all_active_users = CustomUser.objects.exclude(
            role__in=["superadmin", "organization"]
        ...
        if organization:
            total_report_count = ReportViewModel.objects.filter(
                organization=organization
            ).count()
            today = timezone.now().date()
            today_report_count = ReportViewModel.objects.filter(
                organization=organization, viewed_at__date=today
            ).count()
        else:
...
class StaffDashboardView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["accounts"] = filtered_accounts
        total_balance = sum(
            account["PBal"] for account in filtered_accounts if "PBal" in account
        )
        context["total_balance"] = total_balance
        return context
        ...
        if response.status_code == 200:
            ...
            if api_response.get("isSuccess"):
                try:
                    ...
                    return JsonResponse(
                        {"isSuccess": False, "error": "Failed to parse account data"}
                    )
            else:
                return JsonResponse(
                    {
                        ...
                        "error": api_response.get(
                            "result", "Failed to retrieve data from API"
                        ),
                    }
                )
        else:
            return JsonResponse(
                {"isSuccess": False, "error": "Failed to retrieve data from API"}
            )
